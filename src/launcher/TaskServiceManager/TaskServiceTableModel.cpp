...
int
TaskServiceTableModel::rowCount(QModelIndex const& parent) const
{
    if (parent.isValid())
    {
        return 0;
...
int
TaskServiceTableModel::columnCount(QModelIndex const& parent) const
{
    if (parent.isValid())
    {
        return 0;
...
Qt::ItemFlags
TaskServiceTableModel::flags(QModelIndex const& index) const
{
    if (!index.isValid())
    {
...
QVariant
TaskServiceTableModel::data(QModelIndex const& index, int role) const
{
    if (!index.isValid() || index.row() < 0 || index.row() >= rowCount())
    {
        return {};
    }
    auto const& controller = m_controllers[static_cast<size_t>(index.row())];
    bool running
    ...
    if (role == Qt::UserRole)
    {
        switch (index.column())
        {
            case ColStatus:
                return running;
            case ColActions:
                return QVariantList { !running, running, true };
            default:
                return {};
        }
    }
...
QVariant
TaskServiceTableModel::headerData(int section, Qt::Orientation orientation, int role) const
{
...
void
TaskServiceTableModel::refreshControllers()
{
    ...
    try
    {
        ...
        for (auto const& controller : controllers)
        {
            // 检查是否已存在
            bool exists = false;
            for (auto const& existingController : m_controllers)
            {
            ...
            {
                // 添加状态回调
                size_t const index = m_controllers.size();
                controller->setStatusCallback(
                    [this, index](service::ITaskServiceManager::ITaskServiceController::TaskServiceStatus status)
                    { onStatusChanged(static_cast<int>(index), status); });
    ...
    {
        common::Log::error(tr("[任务服务] 刷新控制器列表失败：%1").arg(QString::fromUtf8(e.what())).toStdString());
    }
}
...
void
TaskServiceTableModel::onControllerEvent(
    ...
    service::ITaskServiceManager::ControllerEvent event)
{
    switch (event)
    {
        case service::ITaskServiceManager::ControllerEvent::Registered:
        {
            ...
            if (!exists)
            {
                int const newRow = static_cast<int>(m_controllers.size());
                beginInsertRows(QModelIndex(), newRow, newRow);

                // 添加状态回调
                controller->setStatusCallback(
                    [this, newRow](service::ITaskServiceManager::ITaskServiceController::TaskServiceStatus status)
                    { onStatusChanged(newRow, status); });
        ...
        case service::ITaskServiceManager::ControllerEvent::Unregistered:
        {
            ...
            if (index >= 0)
            {
                ...
                common::Log::info(tr("[任务服务] 已注销：%1").arg(name).toStdString());
            }
            break;
        }
    }
...
void
TaskServiceTableModel::onStatusChanged(int index,
                                       service::ITaskServiceManager::ITaskServiceController::TaskServiceStatus status)
{
...
bool
TaskServiceTableModel::isRunning(int row) const
{
    ...
    auto const& controller = m_controllers[static_cast<size_t>(row)];
    return controller->status() == service::ITaskServiceManager::ITaskServiceController::TaskServiceStatus::Running;
}

bool
TaskServiceTableModel::startService(int row)
{
    ...
    {
        common::Log::error(tr("[任务服务] 启动服务失败：%1").arg(QString::fromUtf8(e.what())).toStdString());
        return false;
    }
}

void
TaskServiceTableModel::stopService(int row)
{
    if (row < 0 || row >= m_controllers.size() || !m_taskServiceManager)
    {
        return;
...
TaskServiceTableModel::AddingService(cppmicroservices::ServiceReference<service::ITaskServiceManager> const& reference)
{
    if (m_taskServiceManager)
    {
        return nullptr;
    ...
    {
        common::Log::error(tr("[任务服务] 获取任务服务管理器失败：%1").arg(QString::fromUtf8(e.what())).toStdString());
    }
    return nullptr;
}

void
TaskServiceTableModel::ModifiedService(
...
void
TaskServiceTableModel::RemovedService(cppmicroservices::ServiceReference<service::ITaskServiceManager> const& reference,
                                      std::shared_ptr<service::ITaskServiceManager> const& service)
{
