...
class TaskServiceDockWidget : public KDDockWidgets::QtWidgets::DockWidget
{
  ...
  public:
    explicit TaskServiceDockWidget(cppmicroservices::BundleContext bundleContext, QWidget* parent = nullptr);
  ...
  private:
    ...
    std::shared_ptr<service::ITaskService::IBasicConfig> buildTaskServiceConfig(int row, bool* ok) const;
