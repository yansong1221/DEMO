...
namespace QtImGui
{
    ...
    void
    ImGuiRenderer::initialize(WindowWrapper* window)
    {
        ...
        // };
        io.SetClipboardTextFn = [](void* user_data, char const* text)
        {
            Q_UNUSED(user_data);
        ...
        io.GetClipboardTextFn = [](void* user_data)
        {
            ...
            return (char const*)g_currentClipboardText.data();
        };
    ...
    void
    ImGuiRenderer::renderDrawList(ImDrawData* draw_data)
    {
        ...
        for (int n = 0; n < draw_data->CmdListsCount; n++)
        {
            ...
            glBufferData(GL_ARRAY_BUFFER,
                         ...
                         (GLvoid const*)cmd_list->VtxBuffer.Data,
                         GL_STREAM_DRAW);

            glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, g_ElementsHandle);
            glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                         (GLsizeiptr)cmd_list->IdxBuffer.Size * sizeof(ImDrawIdx),
                         (GLvoid const*)cmd_list->IdxBuffer.Data,
                         GL_STREAM_DRAW);
    ...
    bool
    ImGuiRenderer::createFontsTexture()
    {
        // Select current context
        ImGui::SetCurrentContext(g_ctx);
        ...
        io.Fonts->GetTexDataAsRGBA32(
            &pixels,
            &width,
            &height); // Load as RGBA 32-bits (75% of the memory is wasted, but default font is so small) because it is
    ...
    ImGuiRenderer::createDeviceObjects()
    {
        ...
        GLchar const* vertex_shader = IMGUIRENDERER_GLSL_VERSION "uniform mat4 ProjMtx;\n"
                                                                 "in vec2 Position;\n"
                                                                 "in vec2 UV;\n"
                                                                 "in vec4 Color;\n"
                                                                 "out vec2 Frag_UV;\n"
...
#undef OFFSETOF
    ...
    ImGuiRenderer::newFrame()
    {
        ...
        {
            QPoint const pos = m_window->mapFromGlobal(QCursor::pos());
            io.MousePos = ImVec2(
                pos.x(),
    ...
    void
    ImGuiRenderer::onMousePressedChange(QMouseEvent* event)
    {
    ...
    void
    ImGuiRenderer::onWheel(QWheelEvent* event)
    {
        // Select current context
        ImGui::SetCurrentContext(g_ctx);
    ...
    void
    ImGuiRenderer::updateCursorShape(ImGuiIO const& io)
    {
        ...
        ImGuiMouseCursor const imgui_cursor = ImGui::GetMouseCursor();
        ...
        else
        {
            ...
            auto const cursor_it = cursorMap.constFind(imgui_cursor);
            if (cursor_it != cursorMap.constEnd()) // `Qt::CursorShape` found for `ImGuiMouseCursor`
            {
                Qt::CursorShape const qt_cursor_shape = *(cursor_it);
    ...
    void
    ImGuiRenderer::setCursorPos(ImGuiIO const& io)
    {
        // NOTE: This code will be executed, only if the following flags have been set:
    ...
    bool
    ImGuiRenderer::eventFilter(QObject* watched, QEvent* event)
    {
        ...
        switch (event->type())
        {
            ...
            case QEvent::KeyRelease:
            {
                ...
                if (auto e = dynamic_cast<QKeyEvent*>(event))
                {
                    ...
                    ImGuiIO& io = ImGui::GetIO();
                    io.AddKeyEvent(ImGuiMod_Ctrl, e->modifiers().testFlag(Qt::KeyboardModifier::ControlModifier));
                    io.AddKeyEvent(ImGuiMod_Shift, e->modifiers().testFlag(Qt::KeyboardModifier::ShiftModifier));
                    io.AddKeyEvent(ImGuiMod_Alt, e->modifiers().testFlag(Qt::KeyboardModifier::AltModifier));
                    io.AddKeyEvent(ImGuiMod_Super, e->modifiers().testFlag(Qt::KeyboardModifier::MetaModifier));

                    bool const key_pressed = (event->type() == QEvent::KeyPress);
