...
void LogWidget::addLog(int level, const QString& bundleName, const QString& message)
{
    LogEntry entry;
    ...
    entry.fullText = QString("[%1] [%2] [%3] %4")
                         .arg(timestamp)
...
void LogWidget::appendToDisplay(const LogEntry& entry)
{
...
QTextCharFormat LogWidget::getFormatForLevel(int level) const
{
    switch (level) {
        case 0: return m_errorFormat;
        case 1: return m_warningFormat;
        case 2: return m_infoFormat;
        case 3: return m_debugFormat;
        case 4: return m_traceFormat;
        case 5: return m_auditFormat;
        default: return m_infoFormat;
    }
}

QString LogWidget::levelToString(int level) const
{
    switch (level) {
        case 0: return tr("ERROR");
        case 1: return tr("WARN");
        case 2: return tr("INFO");
        case 3: return tr("DEBUG");
        case 4: return tr("TRACE");
        case 5: return tr("AUDIT");
        default: return tr("UNKNOWN");
    }
}

QColor LogWidget::levelToColor(int level) const
{
    switch (level) {
        case 0: return QColor(220, 53, 69);
        case 1: return QColor(255, 193, 7);
        case 2: return QColor(40, 167, 69);
        case 3: return QColor(108, 117, 125);
        case 4: return QColor(173, 181, 189);
        case 5: return QColor(111, 66, 193);
        default: return QColor(0, 0, 0);
    }
}

bool LogWidget::entryMatchesFilter(const LogEntry& entry) const
{
    switch (entry.level) {
        case 0: return m_showErrorCheck->isChecked();
        case 1: return m_showWarningCheck->isChecked();
        case 2: return m_showInfoCheck->isChecked();
        case 3: return m_showDebugCheck->isChecked();
        case 4: return m_showDebugCheck->isChecked(); // Trace 跟随 Debug
        case 5: return true; // Audit 始终显示
        default: return true;
    }
}
...
void LogWidget::highlightSearchResults()
{
    // 简单的文本高亮实现
...
void LogWidget::onSearchPrevious()
{
    if (m_currentSearchText.isEmpty()) return;
...
void LogWidget::onAutoScrollToggled(bool checked)
{
    Q_UNUSED(checked)
    // 自动滚动状态已更改
}
...
QString LogWidget::getAllLogText() const
{
    std::lock_guard<std::mutex> lock(m_logMutex);
    QString result;
    for (const auto& entry : m_logEntries) {
...
void LogWidget::setMaxLogLines(int maxLines)
{
