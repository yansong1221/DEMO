...
LogServiceActivator::LogServiceActivator()
{
    s_instance = this;
}
...
void LogServiceActivator::Start(cppmicroservices::BundleContext context)
{
    // 创建 LogService 实现
    ...
    // 注册 LogService 服务
    m_serviceRegistration = context.RegisterService<cppmicroservices::logservice::LogService>(
        m_logServiceImpl, props);
    
    // 同时注册 LoggerFactory 服务（LogService 继承自 LoggerFactory）
    m_factoryRegistration = context.RegisterService<cppmicroservices::logservice::LoggerFactory>(
        m_logServiceImpl, props);
    
    // 记录启动日志
    if (m_logServiceImpl) {
        m_logServiceImpl->Log(cppmicroservices::logservice::SeverityLevel::LOG_INFO, 
                              "LogService started successfully");
    }
}

void LogServiceActivator::Stop(cppmicroservices::BundleContext context)
{
    (void)context;
...
LogServiceImpl* LogServiceActivator::GetLogServiceImpl()
{
