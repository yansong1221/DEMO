...
TaskServiceActionDelegate::TaskServiceActionDelegate(QObject* parent)
    : QStyledItemDelegate(parent) {
}

QRect TaskServiceActionDelegate::startStopButtonRect(const QRect& cell) {
    const QRect inner = cell.adjusted(kMargin, 2, -kMargin, -2);
    return QRect(inner.left(), inner.top(), kButtonWidth, inner.height());
}

QRect TaskServiceActionDelegate::configButtonRect(const QRect& cell) {
    const QRect inner = cell.adjusted(kMargin, 2, -kMargin, -2);
    return QRect(inner.left() + kButtonWidth + kGap, inner.top(), kButtonWidth, inner.height());
}

void TaskServiceActionDelegate::drawActionButton(
    QPainter* painter,
    const QRect& rect,
    const QString& text,
...
void TaskServiceActionDelegate::paint(
    QPainter* painter,
    ...
    if (isRunning) {
        // 运行中：显示红色"停止"按钮
        drawActionButton(painter, startStopButtonRect(option.rect), QObject::tr("停止"),
                         QColor(220, 80, 80),   // 红色背景
                         QColor(255, 255, 255), // 白色文字
                         true);
    } else {
        // 已停止：显示普通"开始"按钮
        drawActionButton(painter, startStopButtonRect(option.rect), QObject::tr("开始"),
    ...
    // 配置按钮
    drawActionButton(painter, configButtonRect(option.rect), QObject::tr("配置"),
                     QColor(240, 240, 240), // 灰色背景
                     QColor(0, 0, 0),       // 黑色文字
                     configEnabled);
}

QSize TaskServiceActionDelegate::sizeHint(const QStyleOptionViewItem& option, const QModelIndex& index) const {
    Q_UNUSED(option);
    Q_UNUSED(index);
    return QSize(kButtonWidth * 2 + kGap + kMargin * 2, 28);
}

bool TaskServiceActionDelegate::editorEvent(
    QEvent* event,
